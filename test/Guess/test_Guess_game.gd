...
func test_load_word_list_returns_array():
	var result = game.load_word_list(WORDS_PATH)
...
func test_word_list_path_is_correct():
	assert_eq(game.WORDS_PATH, "res://Scenes/GuessingGame/a_words.txt", "Word list path should be correct");
	
func test_file_access_success():
	var file = FileAccess.open(game.WORDS_PATH, FileAccess.READ)
	assert_not_null(file,"Should be able to open word list file")
	if file:
		file.close()
		
func test_word_list_no_duplicates():
	var result: Array[String] = game.load_word_list(WORDS_PATH)
...
func test_repeated_loading_consistency():
	var first_load = game.load_word_list(WORDS_PATH)
	var second_load = game.load_word_list(WORDS_PATH)
